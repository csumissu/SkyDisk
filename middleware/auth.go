...
func SignRequired() gin.HandlerFunc {
	return func(c *gin.Context) {
		if token := c.GetHeader("Authorization"); len(token) == 0 {
			response := dto.Failure(http.StatusUnauthorized, "authorization header is missing")
			c.JSON(response.HttpStatus, response)
			c.Abort()
		} else if claims, err := util.ParseJwtToken(config.JwtCfg.SigningKey, token); err != nil {
			response := dto.FailureWithError(http.StatusUnauthorized, "token is invalid", err)
			c.JSON(response.HttpStatus, response)
			c.Abort()
